...
def doctorclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'hospital/medical_aid.html')
...
def Services_offered_click_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'hospital/services_offered.html')
...
def admin_dashboard(request):
    ...
    return render(request,'hospital/admin_dashboard.html', mydict)

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def search_admin_dashboard(request):
    appointments=models.Appointment.objects.all().order_by('-id')
    if request.method == 'POST':
        search_str=json.load(request.body).get('searchText','')

        myapoint_name_starts = appointments.filter(name_of_patient=search_str) | appointments.filter(name_of_doctor=search_str) | appointments.filter(appointmanent_date=search_str)

        data = myapoint_name_starts.values()
...
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_appointment_dashboard_view(request):
    appointments=models.Appointment.objects.all().filter(status=True)
    return render(request,'hospital/admin_dashboard.html', {'appointments':appointments})
